...
void keyboard(unsigned char key, int x, int y) {
	...
	case 't': { // Generate streamlines, singularities, and then more streamlines
		...
		for (int i = -10; i < 10; i++) {
			...
			streamline(lines, icVector3(0, i, 0), 0.001); // d2 was 0.001 but was taking too long to render
			lines.m_rgb = icVector3(1.0, 0.0, 0.0);
	...
	if (button == GLUT_LEFT_BUTTON || button == GLUT_RIGHT_BUTTON) {
		...
		if (state == GLUT_DOWN) {
			...
			else if (button == GLUT_LEFT_BUTTON && key == GLUT_ACTIVE_CTRL)
			{
				...
				GLuint selectBuf[GLUT_INIT_WINDOW_WIDTH];
				GLint hits;
				GLint viewport[4];
